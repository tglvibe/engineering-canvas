    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-foreground">Community Solutions</h3>
        ...
        <span className="text-[10px] text-muted-foreground flex items-center gap-1"><Users className="w-3 h-3" /> 10,000+ learners</span>
      </div>
      ...
      <div className="rounded-lg border border-primary/20 bg-primary/[0.02] p-3 mb-3">
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <span className="text-xs font-semibold text-foreground">AI Summary of Best Approach</span>
        </div>
          ...
          return (
            <div key={sol.id} className={`rounded-lg border bg-card overflow-hidden transition-all ${sol.isTopSolution ? "border-primary/30" : "border-border"}`}>
              <button onClick={() => setExpanded(expanded === sol.id ? null : sol.id)} className="w-full flex items-center gap-3 p-3 text-left">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${sol.isTopSolution ? "bg-primary text-primary-foreground" : "bg-secondary text-foreground"}`}>
                  {user?.avatar}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {sol.isTopSolution && (
                      <span className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-primary/10 text-[10px] font-bold text-primary">
                        <Trophy className="w-3 h-3" /> Top Solution
                      </span>
                    )}
                  </div>
              ...
              {expanded === sol.id && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="px-3 pb-3 border-t border-border">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button className="px-2.5 py-1 rounded-md bg-secondary text-xs font-medium text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1">
