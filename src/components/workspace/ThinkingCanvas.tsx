...
interface Note { id: string; text: string; type: "note" | "flashcard" | "highlight"; aiResponse?: string; createdAt: string; }
...
export default function ThinkingCanvas({ open, onClose, topicTitle }: { open: boolean; onClose: () => void; topicTitle: string }) {
  ...
  const addNote = () => { if (!newNote.trim()) return; setNotes(prev => [{ id: `n${Date.now()}`, text: newNote, type: "note", createdAt: "Just now" }, ...prev]); setNewNote(""); };
  const convertToFlashcard = (id: string) => setNotes(prev => prev.map(n => n.id === id ? { ...n, type: "flashcard" as const } : n));
    ...
    <>
      ...
      <motion.aside initial={{ x: 300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 300, opacity: 0 }} transition={{ type: "spring", damping: 25, stiffness: 300 }}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
          <div className="flex items-center gap-2"><StickyNote className="w-4 h-4 text-primary" /><h3 className="text-sm font-semibold text-foreground">{t("workspace.thinkingCanvas")}</h3></div>
          <div className="flex items-center gap-1">
            <button className="p-1.5 text-muted-foreground hover:text-foreground" title={t("workspace.exportNotes")}><Download className="w-3.5 h-3.5" /></button>
            <button onClick={onClose} className="text-muted-foreground hover:text-foreground"><X className="w-4 h-4" /></button>
          </div>
        </div>
        <div className="p-3 border-b border-border shrink-0">
          <textarea value={newNote} onChange={e => setNewNote(e.target.value)} placeholder={t("workspace.addNoteHighlight")} rows={2}
          ...
          <div className="flex items-center justify-between mt-2">
            <span className="text-[10px] text-muted-foreground">{t("workspace.ctrlEnterSave")}</span>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2 scrollbar-hide">
          {notes.map(note => (
            <motion.div key={note.id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
              className={`rounded-lg p-3 border text-sm ${note.type === "flashcard" ? "border-primary/20 bg-primary/[0.03]" : note.type === "highlight" ? "border-accent/30 bg-accent/5 border-l-4 border-l-accent" : "border-border bg-card"}`}>
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-1.5 mb-1">
                  {note.type === "flashcard" && <BookOpen className="w-3 h-3 text-primary" />}
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                    {note.type === "flashcard" ? t("workspace.flashcard") : note.type === "highlight" ? t("workspace.highlight") : t("workspace.note")}
                  </span>
                  <span className="text-[10px] text-muted-foreground/50">{note.createdAt}</span>
                </div>
                <button onClick={() => deleteNote(note.id)} className="text-muted-foreground/40 hover:text-destructive p-0.5"><Trash2 className="w-3 h-3" /></button>
              </div>
              ...
              {note.aiResponse && (
                <div className="mt-2 p-2 rounded-md bg-primary/5 border border-primary/10">
                  <div className="flex items-center gap-1 mb-1"><Sparkles className="w-3 h-3 text-primary" /><span className="text-[10px] font-semibold text-primary">{t("workspace.aiInsight")}</span></div>
                  <p className="text-[11px] text-muted-foreground leading-relaxed">{note.aiResponse}</p>
                </div>
              )}
              <div className="flex items-center gap-1.5 mt-2">
                {note.type !== "flashcard" && <button onClick={() => convertToFlashcard(note.id)} className="px-2 py-0.5 rounded text-[10px] font-medium bg-secondary text-muted-foreground hover:text-foreground transition-colors">{t("workspace.toFlashcard")}</button>}
                {!note.aiResponse && (
                  <button onClick={() => askAI(note.id, note.text)} disabled={askingAI === note.id}
                    ...
                    {askingAI === note.id ? t("workspace.thinking") : t("workspace.askAI")}
                  </button>
                )}
              </div>
            </motion.div>
