...
export default function KnowledgeGraph({ onClose }: { onClose: () => void }) {
  ...
  const handleWheel = (e: React.WheelEvent) => { e.preventDefault(); setScale(prev => Math.max(0.4, Math.min(2, prev - e.deltaY * 0.001))); };
  const handlePointerDown = (e: React.PointerEvent) => {
    if ((e.target as Element).tagName === "svg" || (e.target as Element).tagName === "line") {
      setDragging(true); setDragStart({ x: e.clientX - pan.x, y: e.clientY - pan.y }); (e.target as Element).setPointerCapture?.(e.pointerId);
    }
  };
  const handlePointerMove = (e: React.PointerEvent) => { if (dragging) setPan({ x: e.clientX - dragStart.x, y: e.clientY - dragStart.y }); };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-background/95 backdrop-blur-sm flex flex-col">
      <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-border">
        <div>
          <h2 className="text-base sm:text-lg font-bold text-foreground">{t("workspace.knowledgeGraph")}</h2>
          <p className="text-[10px] sm:text-xs text-muted-foreground">{t("workspace.knowledgeGraphDesc")}</p>
        </div>
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button onClick={onClose} className="px-3 py-1.5 rounded-lg bg-secondary text-xs sm:text-sm font-medium text-foreground hover:bg-secondary/80">{t("common.close")}</button>
        </div>
      </div>
      <div className="flex-1 relative overflow-hidden touch-none" onWheel={handleWheel} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}>
        <svg ref={svgRef} className="w-full h-full cursor-grab active:cursor-grabbing" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${scale})`, transformOrigin: "center" }}>
          {knowledgeGraphEdges.map((edge, i) => {
            ...
            return <line key={i} x1={from.x + 60} y1={from.y + 20} x2={to.x + 60} y2={to.y + 20} stroke="hsl(0 0% 85%)" strokeWidth="1.5" strokeDasharray="4 4" />;
          })}
          {knowledgeGraphNodes.map(node => (
            <g key={node.id} onClick={() => setSelected(selected?.id === node.id ? null : node)} className="cursor-pointer">
              ...
              <text x={node.x + 60} y={node.y + 18} textAnchor="middle" dominantBaseline="central" className="text-[10px] font-medium fill-current" style={{ fill: "hsl(0 0% 20%)" }}>{node.label}</text>
              <text x={node.x + 60} y={node.y + 30} textAnchor="middle" className="text-[9px] fill-current" style={{ fill: "hsl(0 0% 50%)" }}>{node.mastery}%</text>
            </g>
          ))}
        </svg>
        {selected && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="absolute bottom-4 left-4 right-4 sm:right-auto p-4 rounded-xl border border-border bg-card shadow-lg sm:max-w-xs">
            ...
            <div className="mt-2 flex items-center gap-2">
              <div className="flex-1 h-2 bg-secondary rounded-full overflow-hidden"><div className="h-full rounded-full" style={{ width: `${selected.mastery}%`, background: getMasteryColor(selected.mastery) }} /></div>
              <span className="text-xs font-mono text-muted-foreground">{selected.mastery}%</span>
            </div>
            <p className="text-xs text-muted-foreground mt-2">
              {selected.mastery >= 80 ? t("workspace.mastered") : selected.mastery >= 40 ? t("workspace.inProgress") : t("workspace.needsAttention")}
            </p>
          </motion.div>
        )}
      </div>
