...
export default function VideoResources({ videos }: { videos: VideoResource[] }) {
  ...
  const handleVideoClick = (video: VideoResource) => {
    if (video.type === "premium" && !isUnlocked(video.id)) setUnlockModalVideo(video);
  ...
  const VideoCard = ({ video }: { video: VideoResource }) => {
    ...
    return (
      <button onClick={() => handleVideoClick(video)} className="w-full text-left group rounded-lg border border-border bg-card hover:border-primary/30 hover:shadow-sm transition-all overflow-hidden">
        <div className="aspect-video bg-secondary relative flex items-center justify-center">
          <div className="w-12 h-12 rounded-full bg-foreground/10 flex items-center justify-center group-hover:bg-primary/20 transition-colors">
            {video.type === "premium" && !unlocked ? <Lock className="w-5 h-5 text-primary" /> : <Play className="w-5 h-5 text-foreground/60 group-hover:text-primary transition-colors" />}
          </div>
          ...
          {video.type === "premium" && (
            <div className={`absolute top-2 left-2 px-2 py-0.5 rounded-full text-[10px] font-bold ${unlocked ? "bg-primary/20 text-primary" : "bg-primary text-primary-foreground"}`}>
              {unlocked ? `✓ ${t("workspace.unlocked")}` : t("workspace.premium")}
            </div>
          )}
        </div>
        <div className="p-3">
      ...
      <div className="space-y-6">
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold text-foreground">{t("workspace.publicVideos")}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{publicVideos.map(v => <VideoCard key={v.id} video={v} />)}</div>
        </div>
        <div>
          <div className="flex items-center gap-2 mb-3">
            <Lock className="w-4 h-4 text-primary" />
            <h3 className="text-sm font-semibold text-foreground">{t("workspace.platformAuthorizedVideos")}</h3>
            <span className="px-1.5 py-0.5 rounded-full bg-primary/10 text-[10px] font-bold text-primary">{premiumVideos.length}</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{premiumVideos.map(v => <VideoCard key={v.id} video={v} />)}</div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {selectedVideo && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-foreground/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelectedVideo(null)}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={e => e.stopPropagation()} className="w-full max-w-2xl rounded-xl border border-border bg-card overflow-hidden">
              <div className="aspect-video bg-foreground/5 flex items-center justify-center relative">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">{t("workspace.videoPlayer")}</p>
                  <p className="text-xs text-muted-foreground mt-1">{t("workspace.mockPlayer")}</p>
                </div>
        ...
        {unlockModalVideo && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-foreground/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setUnlockModalVideo(null)}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={e => e.stopPropagation()} className="w-full max-w-sm rounded-xl border border-border bg-card p-5 text-center">
              ...
              <button onClick={() => setUnlockModalVideo(null)} className="w-full py-2.5 rounded-xl border border-border text-sm font-medium text-muted-foreground hover:bg-secondary transition-colors">{t("workspace.maybeLater")}</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
