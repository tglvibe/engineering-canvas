...
import { Brain, X, Send, ToggleLeft, ToggleRight, Sparkles } from "lucide-react";
...
export default function MultiLLMPanel({ open, onClose, topicTitle }: { open: boolean; onClose: () => void; topicTitle: string }) {
  ...
  const sendQuery = () => {
    ...
    if (mode === "single") {
      ...
      // Multi/Consultant mode — stagger responses
    ...
    >
      ...
      <div className="p-3 border-t border-border">
        <div className="flex items-center gap-2">
          ...
          />
          <button onClick={sendQuery} disabled={loading} className="p-2 bg-gradient-brand text-primary-foreground rounded-lg hover:opacity-90 disabled:opacity-50">
