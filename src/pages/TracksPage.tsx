...
const categories = [
  { id: "all" as Category, label: "All Tracks", icon: Layers },
  { id: "high-code" as Category, label: "High Code", icon: Code2 },
  { id: "low-code" as Category, label: "Low Code", icon: Wand2 },
  { id: "no-code" as Category, label: "No Code", icon: Sparkles },
];
...
export default function TracksPage() {
  ...
  // Compute recommendations based on user preferences
  ...
  return (
    <div className="min-h-screen bg-background">
      <AppHeader title="Learning Tracks" showBack={false} />

      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground">Choose Your Track</h1>
          <p className="text-muted-foreground mt-1.5 text-sm sm:text-base">Each track builds one real-world project from start to finish.</p>
        </motion.div>
        ...
        <div className="grid gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      {track.recommended && (
                        <span className="shrink-0 px-1.5 py-0.5 rounded-full bg-primary/10 text-[9px] font-bold text-primary">FOR YOU</span>
                      )}
                    </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-12 text-sm text-muted-foreground">No tracks match your search.</div>
          )}
        </div>
      </div>
    </div>
