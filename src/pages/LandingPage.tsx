    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border">
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <a href="/login" className="text-sm font-medium bg-gradient-brand text-primary-foreground px-4 py-2 rounded-lg hover:opacity-90 transition-opacity">Get Started</a>
          </div>
        </div>
      </nav>

      {/* Hero */}
      <section className="pt-32 pb-20 px-6 bg-gradient-warm">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-secondary text-sm font-medium text-muted-foreground mb-6 border border-border">
            ...
            <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-3">
              <a href="/login" className="inline-flex items-center gap-2 bg-gradient-brand text-primary-foreground font-semibold px-8 py-3.5 rounded-xl shadow-brand hover:opacity-90 transition-all text-base">
                Start Learning <ArrowRight className="w-4 h-4" />
              </a>
              <a href="#tracks" className="inline-flex items-center gap-2 text-foreground font-medium px-8 py-3.5 rounded-xl border border-border hover:bg-secondary transition-colors text-base">
                Explore Tracks
              </a>
            </div>
          </motion.div>
      ...
      <section id="tracks" className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold text-foreground">Engineering Tracks</h2>
            <p className="mt-3 text-muted-foreground text-lg">Choose your path. Build something real.</p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {tracks.map((track, i) => (
              <motion.a
                ...
                className="group p-5 rounded-xl border border-border bg-card hover:border-primary/30 hover:shadow-md transition-all flex gap-4 items-start"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">{track.duration} · {track.roles}</p>
                  <p className="text-xs text-primary font-medium mt-2">🚀 {track.projects}</p>
                </div>
              </motion.a>
            ))}
          </div>
      ...
      <footer className="py-10 px-6 border-t border-border">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-sm text-muted-foreground">Great talent. Global opportunities.</p>
        </div>
      </footer>
    </div>
  );
